...
## ADR-023: Phase 5~7 테스트 생략 후 Phase 8 일괄 통합 검증 전략

- **날짜:** 2026-05-08
- **Phase:** Phase 8
- **결정:** Phase 5(모니터링), Phase 6(생산 라인), Phase 7(출고 처리)의 단위 테스트 파일(`tests/phase5/`, `tests/phase6/`, `tests/phase7/`)을 별도로 작성하지 않고, Phase 8 통합 테스트(`tests/phase8/test_integration.py`) 한 곳에서 세 Phase를 포함한 전체 시스템을 E2E로 일괄 검증한다.
- **이유:**
  - Phase 5~7의 각 서비스 로직은 독립 단위로는 의미가 제한적이며, 실제 가치는 전체 흐름(시료 등록 → 주문 → 승인 → 생산/출고)에서 드러난다.
  - 서비스 레이어 간 상호작용(재고 공유, 상태 전환 연쇄)을 통합 테스트로 한 번에 검증하는 것이 단위 테스트 3개를 별도로 작성하는 것보다 시나리오 기반 품질 보증에 효과적이다.
  - 테스트 작성 비용을 절감하고, 실제 운영 시나리오(A: 재고 부족, B: 재고 충분, C: 거절)를 중심으로 검증 범위를 집중한다.
  - Phase 5~7의 서비스별 예외 케이스(빈 큐, 잘못된 상태 출고, 존재하지 않는 ID 등)는 `TestErrorHandling`과 `TestProductionEdgeCases` 클래스에서 통합 테스트 내에 포함하여 커버리지를 보완한다.
- **결과:** 37개 통합 테스트로 Phase 1~8 전체 서비스 레이어 검증 완료. 누적 102개 테스트 전원 통과.
- **대안:** Phase별 단위 테스트 파일 개별 작성 — 반복적인 fixture 설정과 단순 서비스 호출 테스트 증가로 유지보수 비용이 늘어 배제.
- **영향 파일:** `tests/phase8/__init__.py`, `tests/phase8/test_integration.py`
