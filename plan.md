...
| `src/ui/__init__.py` | 빈 패키지 파일 |
| `src/ui/display.py` | `print_header()`, `print_divider()`, `input_prompt()`, `print_error()` 공통 UI 유틸 |
| `src/ui/sample_menu.py` | 시료 관리 서브메뉴 (등록/조회/검색) |
| `main.py` | 시스템 진입점. Phase 2에서는 `(1) 시료 관리`만 활성화, 나머지는 `(준비 중)` 표시 |
...
**UI 공통 규칙 (display.py 기준으로 모든 Phase 공유):**
- 헤더: `[ 메뉴명 > 서브메뉴명 ]` 형식
- 입력 프롬프트: `>> ` 형식
...
**완료 기준:** 모든 단위 테스트 PASS, 커버리지 90% 이상, `python main.py` 실행 후 시료 관리 기능 정상 동작 확인
...
| `src/ui/order_menu.py` | 시료 주문 화면 (고객이 직접 입력) |
...
| `src/ui/monitoring_menu.py` | 모니터링 화면 (주문량/재고량 조회) |
| `main.py` (수정) | `(4) 모니터링` 메뉴 활성화, 메인 메뉴 요약 정보 표시 (시료 수, 재고 합계, RESERVED/PRODUCING 건수) |
...
| `src/ui/production_menu.py` | 생산 라인 조회 화면 (현황 표시 + 완료 처리 입력) |
